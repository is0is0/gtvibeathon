...
ALWAYS use proper texture coordinates for procedural textures:

```python
# Standard setup for any procedural material
tex_coord = nodes.new(type='ShaderNodeTexCoord')
mapping = nodes.new(type='ShaderNodeMapping')

# Adjust scale, rotation, location
...
links.new(tex_coord.outputs['UV'], mapping.inputs['Vector'])  # For UV-mapped objects
# OR
links.new(tex_coord.outputs['Generated'], mapping.inputs['Vector'])  # For procedural
# OR
links.new(tex_coord.outputs['Object'], mapping.inputs['Vector'])  # For object space

# Then connect mapping to your texture
links.new(mapping.outputs['Vector'], texture_node.inputs['Vector'])
```

**Common Coordinate Types:**
- `UV`: Best for objects with proper UV unwrapping (most objects)
- `Generated`: Good for simple procedural patterns
- `Object`: Good for world-space effects
- `Camera`: For camera-projection effects
- `Normal`: For effects based on surface angle

**Material Variation Strategies:**

1. **Use Color Ramps for Control:**
   - Convert noise/patterns to colors
   - Create gradients and transitions
   - Fine-tune procedural results

2. **Layer Multiple Textures:**
   - Mix Noise + Voronoi for complex patterns
   - Use Wave for directional details
   - Add Musgrave for organic variation

3. **Add Surface Detail:**
   - ALWAYS use Bump or Normal maps
   - Connect noise textures to bump height
   - Adjust strength (0.1-1.0 typically)

4. **Realistic Roughness:**
   - Never use constant roughness
   - Add noise variation (0.3-0.7 range)
   - Metals: low roughness (0.1-0.3)
   - Plastics: medium roughness (0.3-0.5)
   - Wood/Stone: high roughness (0.5-0.9)

**Complete Material Template:**
```python
import bpy

def create_complex_material(name, base_color=(0.8, 0.8, 0.8, 1.0)):
    '''Create a complex procedural material'''
    mat = bpy.data.materials.new(name=name)
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    ...
    color_ramp.color_ramp.elements[1].color = (base_color[0]*0.8, base_color[1]*0.8, base_color[2]*0.8, 1.0)
    ...
    bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    links.new(color_ramp.outputs['Color'], bsdf.inputs['Base Color'])
    links.new(noise_rough.outputs['Fac'], bsdf.inputs['Roughness'])
    links.new(bump.outputs['Normal'], bsdf.inputs['Normal'])
...
# Create and apply materials to all objects
for obj in bpy.data.objects:
    if obj.type == 'MESH' and not obj.data.materials:
        mat = create_complex_material(f"{obj.name}_Material")
        obj.data.materials.append(mat)

print("Materials applied with UV mapping and procedural details")
```

**Requirements:**
- Create materials for EVERY object in the scene
- ALWAYS use texture coordinates (UV or Generated)
- ALWAYS use mapping nodes for scale/rotation control
- Use varied, interesting materials (not all solid colors)
- Mix procedural textures creatively (2-3 textures per material)
- ALWAYS use bump/normal maps for surface detail
- ALWAYS add roughness variation (no constant values)
- Consider the scene's mood when choosing colors and properties
- Apply emission to light-emitting objects
- Use glass/transparency where appropriate
- Create at least 5-10 different material types per scene
